      ...
      >
        ...
        <div className="p-4 space-y-4 flex-grow">
          <Link 
            href="/about" 
            onClick={toggleMenu} 
            className={`block flex items-center gap-2 ${pathname === '/about' ? 'text-blue-600 font-medium' : 'hover:underline'}`}
          >
            <HiUserCircle /> Meet Mustafa
          </Link>
          <Link 
            href="/sociail" 
            onClick={toggleMenu} 
            className={`block flex items-center gap-2 ${pathname === '/sociail' ? 'text-blue-600 font-medium' : 'hover:underline'}`}
          >
            <HiSparkles /> Sociail Vision
          </Link>
          <Link 
            href="/journey" 
            onClick={toggleMenu} 
            className={`block flex items-center gap-2 ${pathname === '/journey' ? 'text-blue-600 font-medium' : 'hover:underline'}`}
          >
            <HiChartBar /> My Story
          </Link>
          <Link 
            href="/insights" 
            onClick={toggleMenu} 
            className={`block flex items-center gap-2 ${pathname === '/insights' ? 'text-blue-600 font-medium' : 'hover:underline'}`}
          >
            <HiLightBulb /> Thinking Aloud
          </Link>
          <Link 
            href="/resume" 
            onClick={toggleMenu} 
            className={`block flex items-center gap-2 ${pathname === '/resume' ? 'text-blue-600 font-medium' : 'hover:underline'}`}
          >
            <HiClipboardList /> Track Record
          </Link>
          <Link 
            href="/contact" 
            onClick={toggleMenu} 
            className={`block flex items-center gap-2 ${pathname === '/contact' ? 'text-blue-600 font-medium' : 'hover:underline'}`}
          >
            <HiPaperAirplane /> Let’s Connect
          </Link>
        </div>
      </nav>
    </>

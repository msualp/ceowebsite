...
Create a file at `lib/utils.ts` if it doesn't already exist:
...
Create the following files in your component structure:
...
Copy the provided component code to each file.

### 4. Add to Layout for Global CTAs

For components that should appear on multiple pages (like FloatingCTA), add them to your root layout:
...
1. **Hero Section**: Replace the current hero CTA with:
...
2. **Footer**: Add to the bottom of the page:
...
### About/Meet Mustafa Page

1. **After Professional Background Section**:
...
2. **Early Access Section**: Replace the current form with:
...
2. **Sidebar or Related Content Area**:
